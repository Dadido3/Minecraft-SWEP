...
function SWEP:UpdateViewmodel()
	...
	if (IsValid(VM)) then 
		...
		if MC.BlockTypes[ GetCSConVarI( "minecraft_blocktype", self:GetOwner() ) ] then
			VM:SetModel( MC.BlockTypes[ GetCSConVarI( "minecraft_blocktype", self:GetOwner() ) ].model )
			VM:SetSkin( GetCSConVarI( "minecraft_blockskin", self:GetOwner() ) )
		end
		--print("new model = "..tostring(MC.BlockTypes[ GetCSConVarI( "minecraft_blocktype", self:GetOwner() ) ].model))
	end
...
function SWEP:Think() 
	...
	if (self.doanim or GetCSConVarB( "mc_viewmodel_animtest", self:GetOwner() ) == true ) then
		--back anim
		...
		--forward anim
		if (self.counter <= 1 and !self.backcheck) then
			--calculate vectorDiff
			self.thePos = - Vector( GetCSConVarF( "mc_viewmodel_dx", self:GetOwner() ),
									GetCSConVarF( "mc_viewmodel_dy", self:GetOwner() ),
...
function SWEP:MCSecondaryAttack()
	...
	if tr.HitNonWorld then
		...
		if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then 
	...
	if isBlock == false and not tr.HitWorld and not tr.HitSky and ( GetCSConVarI( "minecraft_spawntype", self:GetOwner() ) == 2 or GetCSConVarI( "minecraft_spawntype", self:GetOwner() ) == 1) then
		...
		if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("hmc = " .. tostring(hmc)) end
		...
		if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("hmc2 = " .. tostring(hmc2)) end
		
		local toporbottom = 0
		if (hmc2 > 15.9 and hmc2 < 16.1) then -- top OR bottom
			if (hmc < 0) then
				toporbottom = 2
				if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("bottom = " .. tostring(toporbottom)) end 
			end
			if (hmc > 0) then
				toporbottom = 1
				if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("top = " .. tostring(toporbottom)) end
			end
		end
		...
		if (toporbottom == 0) then  --one of the four sides
			if (tr.HitPos.z > checkvec.z) then
				...
				if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("upper half") end
			end
			if (tr.HitPos.z < checkvec.z) then
				zpos = math.floor( SpawnPos.z / 36.5 ) * 36.5 + 0--36.5
				if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("lower half") end
			end
		end
		...
		if (toporbottom == 1) then --top
			...
			if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("top->SpawnPos.z = " .. tostring(SpawnPos.z)) end
			if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("top->zpos = " .. tostring(zpos)) end
		end
		...
		if (toporbottom == 2) then --bottom
			...
			if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("bottom->SpawnPos.z = " .. tostring(SpawnPos.z)) end
			if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("bottom->zpos = " .. tostring(zpos)) end
		end
		
		if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("isBlock == false!!, checkvec.z = " .. tostring(checkvec.z)) end
		if ( ClDebugEnabled() and GetCSConVarB( "minecraft_debug", self:GetOwner() )) then print("HitPos.z = " .. tostring(tr.HitPos.z)) end
	end
	...
	if ( tr.HitWorld == true and isBlock == false and GetCSConVarI( "minecraft_spawntype", self:GetOwner() ) == 2) then 
		zpos = math.floor( SpawnPos.z / 36.5 ) * 36.5 + 0
	...
	if ( isBlock == false ) then  --pos is ok, block can be created
		...
		--set all attributes
		--[[
		ent:SetModel( MC.BlockTypes[blockID].model )
...
function removeselected( ply, cmd, arg )
	...
	for k,v in pairs( MC.BlockTypes ) do
		if (k == GetCSConVarI( "minecraft_blocktype", ply) ) then
